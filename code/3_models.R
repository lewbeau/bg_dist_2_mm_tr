...
summary(m_all_vars) 
#AIC: 1227.8; significant vars (0.5) - province, family_status, education, empl_status. 
options(scipen=999) 
...
m_sign_vars <- glm(marriage_tr ~ family_status + province + education + empl_status, data = train, family = "binomial")
summary(m_sign_vars) 

#Hosmer-Lemeshow (Дейвид Хозмър - Хосмър и Стенли Лемешоу - Лимъшоу) goodness of fit test in R (https://www.youtube.com/watch?v=MYW8gA1EQCQ)
hl2 <- hoslem.test(m_sign_vars$y, fitted(m_sign_vars), g=10)
hl2
#AIC: 1192.9; significant vars (0.5) - all in the model.
#Goodness-of-fit test
...
#Accuracy (train data) - 65.6% beats the 60.22% baseline
(458+134)/(903)
...
#Accuracy (test data) - 61.5% beats the 60.2% baseline
(184+52)/(384) #with regard to overfitting the model is OK; better predicting those who disagree
